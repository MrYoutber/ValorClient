...
public class ModKeystrokes extends ModDraggable {
	...
	public static enum KeystrokesMode {
		...
		WASD_JUMP(Key.W, Key.A, Key.S, Key.D,
				new Key("_______", Minecraft.getMinecraft().gameSettings.keyBindJump, 1, 41, 58, 18)),
		WASD_JUMP_MOUSE(Key.W, Key.A, Key.S, Key.D, Key.LMB, Key.RMB,
				new Key("_______", Minecraft.getMinecraft().gameSettings.keyBindJump, 1, 61, 58, 18));
		...
		for (Key key : mode.getKeys()) {
			...
			if (key.isDown()) {
				Gui.drawRect(pos.getAbsoluteX() + key.getX(), pos.getAbsoluteY() + key.getY(),
						...
						pos.getAbsoluteY() + key.getY() + key.getHeight(), new Color(255, 255, 255, 220).getRGB());
			} else {
				Gui.drawRect(pos.getAbsoluteX() + key.getX(), pos.getAbsoluteY() + key.getY(),
						pos.getAbsoluteX() + key.getX() + key.getWidth(),
						pos.getAbsoluteY() + key.getY() + key.getHeight(), new Color(0, 0, 0, 120).getRGB());
			}
